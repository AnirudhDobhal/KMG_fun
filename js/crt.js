...
let dataParticles = [];
let frequencyBars = [];
const COMMUNICATION_SEQUENCE = ['B', 'C', 'F', 'E', 'B', 'C', 'E', 'D', 'B', 'C', 'F', 'E', 'D', 'C'];
...
function initializeDashboard() {
  ...
  // Start sequence flash hinting
  updateGuide();
  startFlashingSequence();
}

// Update the assistive guide UI
function updateGuide() {
  const instrText = document.getElementById('dynamic-instruction');
  const stepText = document.getElementById('current-step');
  
  if (currentSequenceIndex < COMMUNICATION_SEQUENCE.length) {
    const nextKey = COMMUNICATION_SEQUENCE[currentSequenceIndex];
    if (instrText) instrText.innerHTML = `OK, PRESS <span class="next-key">${nextKey}</span> FIRST...`;
    if (stepText) stepText.textContent = currentSequenceIndex + 1;
  } else {
    if (instrText) instrText.innerHTML = `<span class="next-key">COMMUNICATION SUCCESSFUL!</span>`;
    currentSequenceIndex = 0; // Reset for loop feel if desired, or keep as success
    setTimeout(updateGuide, 3000);
  }
}

// Advance the guide sequence if the correct key is pressed
function advanceSequence(keyChar) {
  if (keyChar === COMMUNICATION_SEQUENCE[currentSequenceIndex]) {
...
// Start hinting sequence over the key elements
function startFlashingSequence() {
  // Flash ONLY the current expected key periodically
  setInterval(() => {
    document.querySelectorAll('.key.hint-flash').forEach(k => k.classList.remove('hint-flash'));
    
    if (currentSequenceIndex < COMMUNICATION_SEQUENCE.length) {
      const keyChar = COMMUNICATION_SEQUENCE[currentSequenceIndex];
      ...
      if (keyEl) {
        keyEl.classList.add('hint-flash');
        setTimeout(() => {
          if (keyEl) keyEl.classList.remove('hint-flash');
        }, 600);
      }
    }
  }, 1200); 
...
function setupKeyPressEvents() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (keys.includes(String.fromCharCode(keyCode))) {
      ...
      // Advance guide sequence if matched
      const isCorrectValue = advanceSequence(key);
